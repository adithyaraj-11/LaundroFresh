      ...
      app.post("/order", async (req, res) => {
        ...
        try {
          ...
          await db
            ...
            .updateOne(
              { regno: regno },
              { $set: { paymentpending: amt, orders: ++user.orders } }
            );
...
app.post("/dashboard", function (req, res) {
  ...
  if (pending == 0) {
    window.alert("No payments pending!");
  } else {
    res.render("payment");
  }
});
...
app.post("/payment", async (req, res) => {
  ...
  await db
    .collection("login")
    .updateOne({ regno: regno }, { $set: { paymentpending: "0" } });
  const users = await db
    .collection("orderhistory")
    .find({ regno: regno })
    .toArray();
  for (let i = 0; i < users.length; i++) {
    if (users[i].status != "Paid") {
      await db
        .collection("orderhistory")
        .updateOne(
          { regno: regno, _id: users[i]._id },
          { $set: { status: "Paid", paidon: currentDate } }
        );
    }
  }
